...
- 2026-09-26 — Mobile navigation/header per the refreshed design: below the mobile breakpoint the header + tab strip are replaced by `MobileHeader` (hamburger, current page title over the point name, "● Приём/○ Стоп" pill) with a left drawer (point name/address, accepting-bookings switch, icon nav with today's not-yet-started count as a badge on Очередь, profile, logout). Nav items are shared in `navItems.ts` (long mobile labels, e.g. "Услуги и цены"); the accepting toggle is shared via `useAcceptingToggle`. Desktop `Header`/`TabBar` unchanged apart from using them. QR-код has no design entry — kept in the drawer with an added icon. Checked in a 390px frame against the dev API; page-level headings ("Услуги" etc.) still repeat the new header title on mobile.
